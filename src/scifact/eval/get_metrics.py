...
### Adapted from https://github.com/allenai/scifact/blob/master/verisci/evaluate/label_prediction.py


def get_predictions(claim_scores, model):
    reorder_by_score_filter_add_softmax


parser = argparse.ArgumentParser()
parser.add_argument("--corpus", type=str, required=True)
parser.add_argument("--datasets", type=str, nargs="+", default=["dev", "test"])
...
print(
    f'Macro F1 w/o NEI:  {f1_score(true_labels, pred_labels, average="macro", labels=[0, 2]).round(4)}'
)
print()
print("                   [C      N      S     ]")
